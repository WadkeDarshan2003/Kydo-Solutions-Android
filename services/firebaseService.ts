...
export const subscribeToDesigners = (callback: (designers: User[]) => void, tenantId?: string): Unsubscribe => {
  // Always query all designers, then filter in JavaScript to support both tenantId and tenantIds
  let q = query(collection(db, "designers"));
  
  return onSnapshot(q, (snapshot) => {
    let designers = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as User));
    
    // Filter by tenantId if provided (handles both single tenantId and tenantIds array)
    if (tenantId) {
      designers = designers.filter(designer => 
        designer.tenantId === tenantId || 
        (designer.tenantIds && Array.isArray(designer.tenantIds) && designer.tenantIds.includes(tenantId))
      );
    }
...
export const subscribeToVendors = (callback: (vendors: User[]) => void, tenantId?: string): Unsubscribe => {
  // Always query all vendors, then filter in JavaScript to support both tenantId and tenantIds
  let q = query(collection(db, "vendors"));
  
  return onSnapshot(q, (snapshot) => {
    let vendors = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as User));
    
    // Filter by tenantId if provided (handles both single tenantId and tenantIds array)
    if (tenantId) {
      vendors = vendors.filter(vendor => 
        vendor.tenantId === tenantId || 
        (vendor.tenantIds && Array.isArray(vendor.tenantIds) && vendor.tenantIds.includes(tenantId))
      );
    }
...
export const subscribeToClients = (callback: (clients: User[]) => void, tenantId?: string): Unsubscribe => {
  // Always query all clients, then filter in JavaScript to support both tenantId and tenantIds
  let q = query(collection(db, "clients"));
  
  return onSnapshot(q, (snapshot) => {
    let clients = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as User));
    
    // Filter by tenantId if provided (handles both single tenantId and tenantIds array)
    if (tenantId) {
      clients = clients.filter(client => 
        client.tenantId === tenantId || 
        (client.tenantIds && Array.isArray(client.tenantIds) && client.tenantIds.includes(tenantId))
      );
    }
