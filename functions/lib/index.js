...
export const sendEmail = functions.https.onRequest(async (req, res) => {
    ...
    try {
        const { to, recipientName, subject, htmlContent } = req.body;
...
export const sendPushNotification = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        if (process.env.NODE_ENV !== 'production')
            console.log(`✅ Push notification sent to ${recipientId}: ${response.successCount} success, ${response.failureCount} failed`);
        res.status(200).json({
            success: true,
            results: {
